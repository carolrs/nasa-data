...
const SearchVideos = () => {
  ...
  const searchVideos = async () => {
    ...
    try {
      ...
      const videoResults = data.collection.items.filter(item => item.data[0].media_type === 'video');
      const videoResultsWithHref = await Promise.all(videoResults.map(async (item) => {
